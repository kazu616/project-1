                    ...
                    <div class="flex flex-col flex-1 gap-5 pr-5 border-r border-[#BCBCBC]">
                        <div class="flex items-center justify-between">
                            ...
                            <input type="text" value="<?= $account['name'] ?>" name="name"
                                class="px-5 py-2 rounded-[10px] min-w-[320px]">
                        </div>
                        <div class="flex items-center justify-between">
                            <span>Email</span>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="relative min-w-[320px]">
                                <input type="password" value="<?= $account['password'] ?>" name="password"
                                    class="px-5 py-2 rounded-[10px] w-full pr-11">
                                <i
                                    class="absolute -translate-y-1/2 fa-regular fa-eye-slash top-1/2 right-3 hide_eyes"></i>
                            </div>
                        </div>
                        <div class="flex items-center justify-between">
                            <span>Phone Number</span>
                            <input type="text" value="<?= $account['phoneNumber'] ?>" name="phone_number"
                                class="px-5 py-2 rounded-[10px] min-w-[320px]">
                        </div>
                        <div class="flex justify-between">
                            <span>Address</span>
                            <textarea class="px-5 py-2 rounded-[10px] min-w-[320px] min-h-[150px] resize-none"
                                name="address"><?= $account['address'] ?></textarea>
                        </div>
                        <button class="px-5 py-2 bg-[#FFA800] text-white font-semibold self-start rounded-[10px]"
                            type="submit">Save</button>
                    </div>
                    <div class="w-[200px] mt-2 flex flex-col justify-center">
                        <img src="imgs/<?= $account['img'] ?>"
                            class="preview_image object-cover h-[150px] w-[150px] mx-auto rounded-full" alt="">
                        <input type="file" id="product-image" name="image" class="hidden">
                        <input type="hidden" value="<?= $account['img'] ?>" name="old_img">
                        <button class="px-10 py-2 bg-[#BEBEBE] text-white rounded-[10px] mt-5 uppercase" type="button"
                            id="upload_avtImg">Uploading</button>
                    </div>
                </div>
            </form>
        </div>
