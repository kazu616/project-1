...
if (minus_amount_orders) {
  minus_amount_orders.forEach((minus_amount_order) => {
    minus_amount_order.addEventListener("click", (e) => {
      ...
      if (amount <= 1) {
        ...
        window.location.href = `?controller=orderAdmin&action=changeAmount&id=${id}&func=minus#${id}`;
      }
      return;
    });
  });
